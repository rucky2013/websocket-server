...
import com.company.websocket.server.WebSocketServerHandler;
 ...
 */
@Component
public class ScanRecordConsumer/* implements InitializingBean*/{
    @Autowired
    private WebSocketServer webSocketServer;

    private static final Logger LOG = LoggerFactory.getLogger(ScanRecordConsumer.class.getName());
...
//                map.clear();
                ...
                for (ConsumerRecord<String, Object> record : records) {
                    System.out.println(k.getKey().partition() + "\t" + record.offset() + "\t" + record.key() + "\t" + record.value());
                    ...
                    ChannelContainer.GLOBAL_CHANNEL_MAP.forEach((reqId, callBack) -> { // 将消息发送到所有机器
                        ...
                        serviceRequest.setMessage(record.value().toString());
                        try {
